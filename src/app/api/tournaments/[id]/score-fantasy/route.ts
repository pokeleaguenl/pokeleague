...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  for (const squad of squads) {
    const activePts = deckPointsMap.get(squad.active_deck_id ?? 0) ?? 0;
    const benchIds = [squad.bench_1, squad.bench_2, squad.bench_3, squad.bench_4, squad.bench_5].filter((id): id is number => id != null);
    const handIds = [squad.hand_1, squad.hand_2, squad.hand_3, squad.hand_4].filter((id): id is number => id != null);
    ...
    if (squad.event_effect === "x3" && !squad.x3_effect_used) activeMultiplier = 3;
    let pts = (activePts * activeMultiplier) + benchPts;
    if (squad.event_effect === "hand_boost" && !squad.hand_boost_used) pts += handPts;

    if (pts === 0) continue;
    scoredCount++;

    await supabase.from("league_scores").upsert(
      { user_id: squad.user_id, tournament_id: numId, points_earned: pts },
      { onConflict: "user_id,tournament_id" }
    );

    const { data: allScores } = await supabase
      .from("league_scores").select("points_earned").eq("user_id", squad.user_id);
    const total = (allScores ?? []).reduce((sum, sc) => sum + (sc.points_earned ?? 0), 0);

    await supabase.from("squads").update({ total_points: total }).eq("user_id", squad.user_id);
    await supabase.from("profiles").update({ total_points: total }).eq("id", squad.user_id);

    if (squad.event_effect === "x3" && !squad.x3_effect_used)
      await supabase.from("squads").update({ x3_effect_used: true, event_effect: null }).eq("user_id", squad.user_id);
    else if (squad.event_effect === "hand_boost" && !squad.hand_boost_used)
      await supabase.from("squads").update({ hand_boost_used: true, event_effect: null }).eq("user_id", squad.user_id);
  }
