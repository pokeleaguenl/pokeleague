...
async function scrapeFullStandings(labsCode) {
  ...
  try {
    console.log(`📥 Navigating to Limitless Labs page ${labsCode}...`);
    
    await page.goto(`https://labs.limitlesstcg.com/${labsCode}/standings`, {
...
async function main() {
  const RK9_ID = 'HO01w20W3tGb1aAsLzwS';
  const TOURNAMENT_ID = 265;
  const LABS_CODE = '0058';
  
  console.log('🎯 HOUSTON REGIONAL IMPORT');
  console.log('==========================================\n');
  console.log(`Tournament ID: ${TOURNAMENT_ID}`);
  console.log(`RK9 ID: ${RK9_ID}`);
  console.log(`Limitless Labs: ${LABS_CODE}\n`);
  
  try {
    const standings = await scrapeFullStandings(LABS_CODE);
    await importStandings(RK9_ID, standings);
    await scoreAllSquads(TOURNAMENT_ID, RK9_ID);
    await syncProfiles();
    
    console.log('\n🎉 ALL DONE! Houston imported, scored, history logged, and profiles synced.');
